...
public final class HuffmanCompress {
	...
	public static void main(String[] args) throws IOException {
		...
		if (args.length == 0) {
			System.err.println("Usage: java HuffmanCompress InputFile OutputFile WriteCanonicalCode(true/false) <CanonicalHeaderFile>");
			System.exit(1);
			return;
		}
		...
		boolean canonicalCodeInSeperateFile = ! Boolean.valueOf(args[2]);
		...
		BitOutputStream out = new BitOutputStream(new BufferedOutputStream(new FileOutputStream(outputFile, false)));
		try {
			
			if(canonicalCodeInSeperateFile)
			{
				//will write the canonical header to a seperate file given as the 4th argument in command line
				String canonicalHeaderFile = args[3];
				BitOutputStream canonicalOut = new BitOutputStream(new BufferedOutputStream(new FileOutputStream(canonicalHeaderFile, false)));
				
				writeCode(canonicalOut, canonCode);
